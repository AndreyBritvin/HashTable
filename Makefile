...
SRC_FILES     = $(wildcard src/*.cpp) $(wildcard src/gui/*.cpp) $(wildcard src/hashtable_calc/*.cpp)
...
hashtable.out:$(SRC_FILES) $(BUILD_FILES)
	@$(CC) $(CFLAGS_DEBUG) $(INCLUDE) $(SRC_FILES) $(MY_LIBS) -O3 -o hashtable.out

full_optimise:
	@$(CC) $(CFLAGS) $(INCLUDE) $(SRC_FILES) $(MY_LIBS) -O3 -o hashtable.out
